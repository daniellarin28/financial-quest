...
function checkMonthComplete(month, monthDiv) {
  ...
  if (tasks && Object.values(tasks).every(Boolean)) {
    if (!medals[month]) {
      medals[month] = true;
      showMedal(`${month} — Медаль за выполнение месяца`);
      launchConfetti();
    }
    monthDiv.classList.add("completed");
  } else {
    // Убираем медаль, если все задачи сняты
    if (medals[month]) {
      delete medals[month];
      renderMedals(); // Перерисовываем медали
    }
...
function showMedal(text) {
  ...
  medalDiv.style.backgroundColor = 'hsl(' + Math.random() * 360 + ', 100%, 50%)';
...
function renderMedals() {
  ...
  Object.keys(medals).forEach(m => {
    if (medals[m]) showMedal(`Медаль за ${m}`);
  });
}
...
function renderMonths() {
  ...
  months.forEach((month, index) => {
    ...
    const categories = [
      ...
      'Обязательные платежи: Placet: 150€, Ferratum: 130€, Credit24: 60€, и другие',
      'Досрочная оплата кредита: 200€',
